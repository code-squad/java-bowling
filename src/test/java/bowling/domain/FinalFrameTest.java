...
public class FinalFrameTest {
    ...
    public void 프레임_준비_Final_Score_스패어_거터_2개_까지만_add() {
        FinalFrame strike = new FinalFrame().add(new Roll(10));
        assertEquals(OutPutView.frameFormat("X"), strike.toString());
        FinalFrame strikeStrike = strike.add(new Roll(10));
        assertEquals(OutPutView.frameFormat("X|X"), strikeStrike.toString());
        FinalFrame strikeStrikeStrike = strike.add(new Roll(10));
        assertEquals(OutPutView.frameFormat("X|X|X"), strikeStrikeStrike.toString());

        FinalFrame gutter = new FinalFrame().add(new Roll(0));
        FinalFrame gutterSpare = gutter.add(new Roll(10));
        assertEquals(OutPutView.frameFormat("-|/"), gutterSpare.toString());
        FinalFrame gutterSpareGutter = gutterSpare.add(new Roll(0));
        assertEquals(OutPutView.frameFormat("-|/|-"), gutterSpareGutter.toString());

        FinalFrame two = new FinalFrame().add(new Roll(2));
        FinalFrame twoSpare = two.add(new Roll(8));
        assertEquals(OutPutView.frameFormat("2|/"), twoSpare.toString());
        FinalFrame twoSpareEight = twoSpare.add(new Roll(8));
    ...
    public void 프레임_준비_Final_Score_Strike_add_스패어가_아닐시_보너스_지급_없음() {
        FinalFrame strike = new FinalFrame().add(new Roll(10));
        assertEquals(OutPutView.frameFormat("X"), strike.toString());
        FinalFrame strikeStrike = strike.add(new Roll(0));
        assertEquals(OutPutView.frameFormat("X|-"), strikeStrike.toString());
        FinalFrame illegalArgumentException = strike.add(new Roll(10));
    }
}